#!/usr/bin/env bash
# Gerencia o ciclo de vida do EvoTeste web (dev/prod) em background.
# Uso:
...
start_prod() {
  if [ -f "$PID_FILE" ] && kill -0 "$(cat "$PID_FILE" | awk '{print $2}')" 2>/dev/null; then
    echo "[evoteste-web] já está rodando (PID $(cat "$PID_FILE" | awk '{print $2}'))"
    return 0
  fi
  if [ ! -d "$ROOT_DIR/node_modules" ]; then
    echo "[evoteste-web] node_modules ausente — rodando npm install..."
    (cd "$ROOT_DIR" && npm install)
  fi
  if [ ! -d "$ROOT_DIR/.next" ]; then
    echo "[evoteste-web] build ausente — gerando..."
    (cd "$ROOT_DIR" && npx next build)
  fi
  if [ ! -f "$ROOT_DIR/.env" ] && [ -f "$ROOT_DIR/../.env" ]; then
    cp "$ROOT_DIR/../.env" "$ROOT_DIR/.env" && chmod 600 "$ROOT_DIR/.env"
  fi
  echo "[evoteste-web] iniciando em produção (porta $PORT)..."
  ...
  if [ -f "$PID_FILE" ]; then
    ...
    echo "[evoteste-web] rodando em http://localhost:$PORT (PID $PID)"
  fi
}

start_dev() {
  if [ -f "$PID_FILE" ] && kill -0 "$(cat "$PID_FILE" | awk '{print $2}')" 2>/dev/null; then
    echo "[evoteste-web] já está rodando — pare antes com: bash dev.sh stop"
    return 1
  fi
  if [ ! -d "$ROOT_DIR/node_modules" ]; then
    (cd "$ROOT_DIR" && npm install)
  fi
  echo "[evoteste-web] iniciando em dev mode (hot reload)..."
  (cd "$ROOT_DIR" && PORT="$PORT" nohup npx tsx watch server.ts > "$LOG_FILE" 2>&1 & echo "PID: $!" > "$PID_FILE")
  sleep 3
  if [ -f "$PID_FILE" ]; then
    PID=$(cat "$PID_FILE" | awk '{print $2}')
    echo "[evoteste-web] dev mode em http://localhost:$PORT (PID $PID)"
  fi
}

stop() {
  if [ ! -f "$PID_FILE" ]; then
    echo "[evoteste-web] não está rodando"
    return 0
  fi
  PID=$(cat "$PID_FILE" | awk '{print $2}')
  if kill -0 "$PID" 2>/dev/null; then
    echo "[evoteste-web] parando PID $PID..."
    ...
    if kill -0 "$PID" 2>/dev/null; then
      echo "[evoteste-web] forçando kill -9..."
  ...
  rm -f "$PID_FILE"
  echo "[evoteste-web] parado"
}

status() {
  if [ -f "$PID_FILE" ] && kill -0 "$(cat "$PID_FILE" | awk '{print $2}')" 2>/dev/null; then
    PID=$(cat "$PID_FILE" | awk '{print $2}')
    echo "[evoteste-web] RODANDO (PID $PID, porta $PORT)"
