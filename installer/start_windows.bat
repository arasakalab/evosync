...
if not exist "%EVOLUTION_ENV%" (
  echo [DisparoFacil] infra\evolution\.env nao encontrado. Rode installer\install_windows.ps1 primeiro.
  exit /b 1
)

docker compose --env-file "%EVOLUTION_ENV%" -f "%EVOLUTION_DIR%\compose.yaml" up -d
if errorlevel 1 (
  echo [DisparoFacil] Nao foi possivel subir a stack Docker. Verifique se o Docker Desktop esta aberto e se a porta 8080 esta livre.
  exit /b 1
)
