...
if [ -f "$EVOLUTION_ENV" ]; then
  if ! docker compose --env-file "$EVOLUTION_ENV" -f "$EVOLUTION_DIR/compose.yaml" up -d; then
    printf '[DisparoFacil] Nao foi possivel subir a stack Docker. Verifique se o Docker esta aberto e se a porta 8080 esta livre.\n' >&2
    exit 1
  fi
else
  printf '[DisparoFacil] infra/evolution/.env nao encontrado. Rode installer/install_linux.sh primeiro.\n' >&2
  exit 1
fi
